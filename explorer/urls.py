...
from explorer.views import (QueryView, CreateQueryView, PlayQueryView, DeleteQueryView, ListQueryView, ListQueryLogView,
                            view_csv_query, email_csv_query, download_query, download_csv_from_sql, schema, format_sql)

urlpatterns = [
    url(r'(?P<query_id>\d+)/$', QueryView.as_view(), name='query_detail'),
    url(r'(?P<query_id>\d+)/download$', download_query, name='query_download'),
    url(r'(?P<query_id>\d+)/csv$', email_csv_query, name='query_csv'),
    url(r'(?P<query_id>\d+)/email_csv$', view_csv_query, name='email_csv_query'),
